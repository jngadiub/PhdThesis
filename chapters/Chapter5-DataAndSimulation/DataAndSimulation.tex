...
where the index $n$ runs over the perturbative order and $s$ is the squared center of mass energy of the collision.
...
Perturbative calculations in QCD are limited to processes in which the coupling constant $\alpha_S^n(\mu_R^2)$ is small, and by the complexity of higher order calculations preventing their evaluation.
...
As last step the pileup due to additional pp collisions in the same bunch crossing is also accounted for.
...
\subfigure[]{\label{fig:pu_mc_data_b}\includegraphics[width=0.45\textwidth]{\chfive/can_h_nVtx.pdf}}\\
\subfigure[]{\label{fig:pu_mc_data_c}\includegraphics[width=0.45\textwidth]{\chfive/PU-data-mc-13TeV.pdf}}
\subfigure[]{\label{fig:pu_mc_data_d}\includegraphics[width=0.45\textwidth]{\chfive/nPV_0.pdf}}
\caption{blabla.}
...
angle parameter that determines \Wpr couplings (\FIXME{ref}) and that cot2$\theta$ and cot$\theta$ can be directly related to $c_\mathrm{H}$ and $c_\mathrm{F}$.
...
The widths for the HVT model B are computed by means of Equation (2.31) in Ref.~\cite{Pappadopulo:2014qza}\FIXME{ref to the theory chapter},
...
Figure~\ref{fig:WHmodelsWidth} shows the ratio between the resonance natural width and mass for a \Wpr in the Little Higgs and the HVT model B.
...
Figure~\ref{fig:allmodelsXsec} compare the production cross sections $\sigma(\mathrm(pp))\rightarrow X$ of the resonance for $\sqrt{s} = 8$ and 13\TeV,
...
\begin{table}[!htb]
 ...
 \resizebox{\textwidth}{!}{
   ...
   \hline
   \hline
   W+jets, W $\rightarrow\ell\Pgn$, $100 < \HT < 200\GeV$ & 1627.5 (NNLO) & \amcatnlo{}\\
   W+jets, W $\rightarrow\ell\Pgn$, $200 < \HT < 400\GeV$ & 435.2 (NNLO) & \amcatnlo{}\\
   W+jets, W $\rightarrow\ell\Pgn$, $400 < \HT < 600\GeV$ & 59.2 (NNLO) & \amcatnlo{}\\
   W+jets, W $\rightarrow\ell\Pgn$, $600 < \HT < 800\GeV$ & 14.6 (NNLO) & \amcatnlo{}\\
   W+jets, W $\rightarrow\ell\Pgn$, $800 < \HT < 1200\GeV$ & 6.7 (NNLO) & \amcatnlo{}\\
   W+jets, W $\rightarrow\ell\Pgn$, $1200 < \HT < 2500\GeV$ & 1.6 (NNLO) & \amcatnlo{}\\
   W+jets, W $\rightarrow\ell\Pgn$, $\HT > 2500\GeV$ & 0.04 (NNLO) & \amcatnlo{}\\ 
   \hline 
   \ttbar (inclusive) & 831.8 (NNLO+NNLL) & \POWHEG{}\\
   \hline   
   single t quark (t-channel), W $\rightarrow\ell\Pgn$ & 44.5 (NNLO) & \POWHEG{}\\
   single $\bar{\mathrm{t}}$ quark (t-channel), W $\rightarrow\ell\Pgn$ &  26.5 (NNLO) & \POWHEG{}\\       
   single t quark (tW-channel, inclusive) & 35.9 (NNLO) & \POWHEG{}\\
   single $\bar{\mathrm{t}}$ quark (tW-channel, inclusive) & 35.9 (NNLO) & \POWHEG{}\\   
   single t+$\bar{t}$ quark (s-channel), W $\rightarrow\ell\Pgn$ & 3.7 (NNLO) & \amcatnlo{}\\   
   \hline
   WW $\rightarrow\ell\Pgn\qqbar^\prime$ & 50.0 (NNLO) & \POWHEG{}\\ 
   WZ $\rightarrow\ell\Pgn\qqbar$ & 10.7 (NLO) & \amcatnlo{}\\ 
   ZZ $\rightarrow\ell\ell\qqbar$ & 3.22 (NLO) & \amcatnlo{}\\ 
   \end{tabular}}
...
All events that are accepted by a specific set of high level triggers enter one specific dataset, so that the choice of a trigger for the analysis defines which dataset has to be used.
